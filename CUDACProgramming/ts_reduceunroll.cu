...
{
    // set thread ID
    unsigned int tid = threadIdx.x;                               // 获取当前线程在块内的索引
    unsigned int idx = blockDim.x * blockIdx.x * 2 + threadIdx.x; // 计算全局索引，*2是因为每个块处理两个元素
    // boundary check
    if (tid >= n)
        return; // 如果线程索引大于或等于数组大小，则结束该线程的执行
    // convert global data pointer to the
    ...
    // in-place reduction in global memory
    for (int stride = blockDim.x / 2; stride > 0; stride >>= 1) // 使用循环进行归约操作，步长每次减半
    {
        if (tid < stride)
        {
            idata[tid] += idata[tid + stride]; // 只有当线程索引小于当前步长时，才执行相加操作
        }
        // synchronize within block
        __syncthreads(); // 每次归约操作后同步线程，确保所有线程都完成了相加操作
    }
    // write result for this block to global mem
    if (tid == 0)
        g_odata[blockIdx.x] = idata[0]; // 如果当前线程是块内的第一个线程（tid == 0），则将归约结果写入输出数组
}
