...
__global__ void copyRow(float *MatA, float *MatB, int nx, int ny) // 行读取，行存储来复制矩阵(上限)
{
...
__global__ void copyCol(float *MatA, float *MatB, int nx, int ny) // 列读取，列存储来复制矩阵(下限)
{
    int ix = threadIdx.x + blockDim.x * blockIdx.x;
    int iy = threadIdx.y + blockDim.y * blockIdx.y;
    int idx = ix * ny + iy;
